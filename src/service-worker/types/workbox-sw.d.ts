// copied from https://github.com/wessberg/workbox/blob/0c3721bcc3188f4f8c1080ab9b054e25585c1b43/packages/workbox-sw/types/workbox-sw.d.ts
// should be removed in latest versions of workbox 
declare module "workbox-sw" {
	 ...
	 */
	type BroadcastCacheUpdateConstructor = {
	 ...
	 */
	type CacheableResponseConstructor = {
	 ...
	 */
	type CacheExpirationConstructor = {
	 ...
	 */
	type StaleWhileRevalidateConstructor = {
	...
	export interface IMatchContext extends IURLContext {
	 ...
	 */
	export type MatchCallback = (context: IMatchContext) => {}|null;
	 ...
	 */
	type NavigationRouteConstructor = {
	 ...
	 */
	type BroadcastUpdatePluginConstructor = {
	 ...
	 */
	type RangeRequestsPluginConstructor = {
	 ...
	 * If both statuses and headers are specified, then both conditions must be met for the Response to be considered cacheable.
	 */
	type CacheableResponsePluginConstructor = {
	 ...
	 */
	type BackgroundSyncPluginConstructor = {
	 ...
	 * When using maxAgeSeconds, requests may be used once after expiring because the expiration clean up will not have occurred until after the cached request has been used. If the request has a "Date" header, then a light weight expiration check is performed and the request will not be used immediately.
	 ...
	 */
	type ExpirationPluginConstructor = {
	 ...
	 */
	type StorableRequestConstructor = {
	 ...
	 */
	type PrecacheControllerConstructor = {
	...
	class Queue {
		...
		/**
		 * Retrieves all stored requests in IndexedDB and retries them. If the queue contained requests that were successfully replayed, the queueDidReplay callback is invoked (which implies the queue is now empty).
	 ...
	 * The "match" callback determine if a route should be used to "handle" a request by returning a non-falsy value if it can. The "handler" callback is called when there is a match and should return a Promise that resolves to a Response.
	 */
	class Route {
